<template>
  <div class="manage-table-box">
    <el-row
      v-if="enableColShowSetting || $slots.searchBar"
    ...
    >
      ...
      >
        <template
          v-if="col.nodeExpressHeaderLabel"
          #header="scope"
        >
          <v-node-render
            :col="col"
...
export default {
  ...
  computed: {
    ...
    visibleColumnDefinition() {
      return this.computedColumnDefinition.filter((item) => item.visible !== false);
    },
    tableData: {
      get() {
    ...
    editableField() {
      return this.computedColumnDefinition.filter((item) => item.editable);
    },
    labelWidthAuto() {
      let width = 36 + 12;
      this.computedColumnDefinition.forEach((item) => {
  ...
  methods: {
    ...
    colShowSettingCheckOneHandle(val, index) {
      ...
      let trueCount = 0;
      let falseCount = 0;
      for (const colState in this.innerComponentStatus.tableColShowFlag) {
    ...
    pageSizeChangeHandle(val) {
      const { pagination } = this.innerComponentStatus;
